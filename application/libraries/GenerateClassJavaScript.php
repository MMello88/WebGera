...
";
  ...
  private function buildJSChild($tabelaFilho){
  ...
  _createClass(DT{$nomeClass}, [{
    ...
    value: function table() {
      return \$('#myTable{$nomeClass}').DataTable({
        ...
        autoWidth: false,
        ajax: url_get_{$table->TABLE_NAME},
        deferRender: true,
        order: [{$lastNum}, 'desc'],
        columns: [
  {$columns}
        ...
        columnDefs: [{
          ...
          render: function render(data, type, row, meta) {
            return `
            <a class='btn btn-sm btn-icon btn-secondary' href='\${url_view_{$table->TABLE_NAME}}/\${data}'>
              <i class='fa fa-eye'></i>
            </a>
